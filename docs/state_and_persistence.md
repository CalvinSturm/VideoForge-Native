...
| Python run manifest | Backend optional artifact | Output-adjacent `.videoforge_runs/<job_id>/` | Per run | Durable when enabled | `src-tauri/src/run_manifest.rs` |
...
`Current truth`:
- Run manifest support exists for the Python path through `maybe_write_run_manifest`
- The app command currently calls Python jobs with `enable_run_artifacts: false`
- No equivalent native run artifact contract was found in the inspected native path
