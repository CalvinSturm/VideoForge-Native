...
fn main() {
    ...
    let protocol_json =
        fs::read_to_string("../ipc/shm_protocol.json").expect("Failed to read shm_protocol.json");

    let json: serde_json::Value =
        serde_json::from_str(&protocol_json).expect("Failed to parse shm_protocol.json");

    let mut content =
        String::from("// Auto-generated SHM constants from ipc/shm_protocol.json\n\n");
    ...
    let pixel_fmt = json["pixel_format_rgb24"]
        .as_u64()
        .expect("pixel_format_rgb24 missing");
    content.push_str(&format!(
        "pub const PIXEL_FORMAT_RGB24: u32 = {};\n",
        pixel_fmt
    ));

    let global_hdr = json["global_header_size"]
        .as_u64()
        .expect("global_header_size missing");
    content.push_str(&format!(
        "pub const GLOBAL_HEADER_SIZE: usize = {};\n",
        global_hdr
    ));

    let slot_hdr = json["slot_header_size"]
        .as_u64()
        .expect("slot_header_size missing");
    content.push_str(&format!(
        "pub const SLOT_HEADER_SIZE: usize = {};\n",
        slot_hdr
    ));
    ...
    content.push_str(&format!(
        "pub const SLOT_HEADER_REGION: usize = {};\n",
        slot_hdr_region
    ));

    let header_region_size = global_hdr + slot_hdr_region;
    content.push_str(&format!(
        "pub const HEADER_REGION_SIZE: usize = {};\n",
        header_region_size
    ));
