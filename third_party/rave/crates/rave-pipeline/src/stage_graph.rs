...
///
/// Controls how many frames are grouped into a single TensorRT inference call.
/// Higher `max_batch` amortizes kernel launch overhead at the cost of per-frame latency.
#[derive(Clone, Debug, Serialize, Deserialize)]
...
mod tests {
    ...
    #[test]
    fn graph_validation_accepts_micro_batching() {
        let mut stage = enhance_stage(1);
        let StageConfig::Enhance { config, .. } = &mut stage else {
            panic!("expected enhance stage");
        };
        config.batch_config.max_batch = 4;

        let graph = StageGraph {
            graph_schema_version: GRAPH_SCHEMA_VERSION,
            stages: vec![stage],
        };
        graph.validate().expect("max_batch=4 should be accepted");
    }

    #[test]
    fn graph_validation_rejects_excessive_batch() {
        let mut stage = enhance_stage(1);
        let StageConfig::Enhance { config, .. } = &mut stage else {
            panic!("expected enhance stage");
        };
        config.batch_config.max_batch = 9;

        let graph = StageGraph {
            graph_schema_version: GRAPH_SCHEMA_VERSION,
            stages: vec![stage],
        };
        let err = graph.validate().expect_err("max_batch=9 must be rejected");
